...
import { Cog, AlertCircle, ArrowRight, ChevronLeft, Save } from "lucide-react";
import Link from "next/link";
import { useParams, useRouter } from "next/navigation";

// These constants would ideally be in a shared file
...
// Enriched dummy data matching the list page, for simulation purposes
const dummySensorTypes = [
  { 
    id: "st-001", 
    name: "Sonde Ambiante THL v2.1", 
    categoryLabels: ["Température", "Humidité"], // Store labels as per create page logic
    description: "Mesure température et humidité ambiantes.",
    examplePayload: { "t": 23.5, "h": 55.2, "bat_v": 3.1 },
    mapping: {"t": "temp", "h": "hum", "bat_v": "battery_voltage"}
  },
  { 
    id: "st-002", 
    name: "Capteur de Pression P-500", 
    categoryLabels: ["Pression"], 
  ...
  { 
    ...
    categoryLabels: ["Qualité de l'air (CO2, VOC, PM)"], 
...
export default function AdminEditSensorTypePage() {
  const params = useParams();
  const router = useRouter();
  const sensorTypeId = params.id as string;
  ...
  useEffect(() => {
    ...
    // Simulate fetching sensor type data
    const sensorTypeToEdit = dummySensorTypes.find(st => st.id === sensorTypeId);

    if (sensorTypeToEdit) {
      setSensorTypeName(sensorTypeToEdit.name);
      // Map category labels back to IDs for checkboxes
      const categoryIds = sensorTypeToEdit.categoryLabels.map(label => {
        const foundCategory = GENERAL_SENSOR_CATEGORIES.find(cat => cat.label === label);
        return foundCategory ? foundCategory.id : "";
      }).filter(id => id !== "");
      setSelectedCategories(categoryIds);
      setSensorDescription(sensorTypeToEdit.description);
      if (sensorTypeToEdit.examplePayload) {
        try {
          const jsonString = JSON.stringify(sensorTypeToEdit.examplePayload, null, 2);
          setExampleJsonText(jsonString);
        } catch (e) {
          console.error("Error stringifying example payload:", e);
          setExampleJsonText("");
        }
      } else {
        setExampleJsonText("");
      }
      setKeyMappings(sensorTypeToEdit.mapping || {});
      setNotFound(false);
    } else {
      setNotFound(true);
    }
  ...
  useEffect(() => {
    if (exampleJsonText.trim() === "") {
      setParsedJsonKeys([]);
      // Don't clear keyMappings if just the text is cleared after initial load
      // setKeyMappings({}); 
      setJsonError(null);
      return;
    }
    try {
      ...
      if (typeof parsed === 'object' && !Array.isArray(parsed) && parsed !== null) {
        ...
        // Preserve existing mappings if keys still exist, otherwise initialize
        const newMappings = { ...keyMappings }; // Start with existing mappings
        keys.forEach(key => {
            if(!newMappings[key]) { // Only set to "" if not already mapped from load
                 newMappings[key] = "";
            }
        });
        // Remove mappings for keys that no longer exist in the new JSON
        ...
        setParsedJsonKeys([]);
        // setKeyMappings({}); // Don't clear on invalid intermediate JSON
        setJsonError("L'exemple JSON doit être un objet JSON valide (pas un tableau ou une primitive).");
      }
    } catch (error) {
      setParsedJsonKeys([]);
      // setKeyMappings({}); // Don't clear on parse error
  ...
  }, [exampleJsonText]); // keyMappings removed from deps to avoid loop on init
  ...
  const handleMappingChange = (jsonKey: string, systemVariableId: string) => {
    setKeyMappings((prevMappings) => {
      ...
      if (systemVariableId === "__NONE__" || systemVariableId === "") {
        newMappings[jsonKey] = ""; 
      } else {
  ...
  const handleUpdateSensorType = () => {
    ...
    console.log("Définition du type de capteur à mettre à jour:", JSON.stringify(sensorTypeData, null, 2));
    alert("Mise à jour du type de capteur simulée. Vérifiez la console pour les données.");
    // Here you would typically send this data to your backend/Firebase
    router.push("/admin/sensors"); // Redirect to list after update
  };

  if (isLoading) {
    return <AppLayout><div className="p-6 text-center">Chargement des données du type de capteur...</div></AppLayout>;
  }

  if (notFound) {
    return <AppLayout><div className="p-6 text-center text-red-500">Type de capteur non trouvé.</div></AppLayout>;
  }
      ...
      <div className="space-y-6">
        <header className="flex items-center justify-between pb-4 mb-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold">Modifier le Type de Capteur</h1>
          </div>
        ...
        <Card className="shadow-xl">
          <CardHeader>
            ...
            <CardDescription>
              Modifiez les détails de ce type de capteur et comment ses données JSON brutes sont interprétées.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
                      ...
                      {parsedJsonKeys.map((key) => (
                        <TableRow key={key}>
                          ...
                          <TableCell className="pl-2">
                            <Select
                              value={keyMappings[key] || "__NONE__"}
            ...
            <div className="flex justify-end pt-6">
              <Button onClick={handleUpdateSensorType} size="lg">
                <Save className="mr-2 h-5 w-5" />
                Mettre à jour le type de capteur
              </Button>
            </div>
          </CardContent>
        </Card>
